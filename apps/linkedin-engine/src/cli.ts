...
program
  ...
  .option("-s, --status <status>", "filter by status (draft|in_review|approved|scheduled|published)")
  ...
  .action((opts) => {
    ...
    posts.forEach((p) => {
      ...
      console.log(`     ${p.language.toUpperCase()} | ${p.status} | ${p.topic} | ${new Date(p.updatedAt).toLocaleDateString()}\n`);
    });
  });
